const QGasToken = artifacts.require("QGasToken");
...
contract('QGasToken', ([alice, bob, carol, james])  => {
    beforeEach(async () => {
      this.instance = await  QGasToken.deployed();
    });
    ...
    it("basic info", async () => {
      ...
      assert.equal(await this.instance.symbol(), "QGas", "wrong symbol"); 
      assert.equal(await this.instance.totalSupply(), 0, "wrong totalSupply");  
      assert.equal(await this.instance.name(), "QGasToken", "wrong name");   
    ...
    it("mint", async () => {
        ...
        let qlcHash = "0x1d3f2eb6d6c73b2c4ca325c8ac18141577761e43abb0154412ef4d36b11ff1b4"
        let packed = solidityPack(
          [ "uint256", "address", "bytes32", "uint256"],
          [ mintAmount, bob, qlcHash, id]
        )
        ...
        //   [ mintAmount]
        // )
        // packed = amountPacked + bob.slice(2) + qlcHash.slice(2)
        // console.log("encode: ", packed) 
        ...
        await chai.assert.isRejected(this.instance.mint(mintAmount, qlcHash,  signHex), 'invalid signature');
        ...
        await chai.assert.isRejected(this.instance.mint(mintAmount, qlcHash, signHex) , 'invalid signature');
        const txResult = await this.instance.mint(mintAmount, qlcHash, signHex, {from: bob})

        truffleAssert.eventEmitted(txResult, 'Mint', (ev) => {
          assert.equal(ev.user, bob, "error user");
          assert.equal(ev.qlcHash, qlcHash, "error hash");
        ...
        await chai.assert.isRejected(this.instance.mint(mintAmount, qlcHash, signHex, {from: bob}), 'duplicated hash');
    ...
    it("burn", async () => {
         ...
         const vaildNep5Addr = "qlc_1b46c8qbkt7azr1sqm4rknyprw6pitpdpwdmyiiswctxfu9f6y4hcmh48esn"
         const invaildNep5Addr = "qlc_1b46c8qbkt7azr1sqm4rknyprw6pitpdpwdmyiiswctxfu9f6y4hcmh48e"

         const txResult = await this.instance.burn(vaildNep5Addr, burnAmount, {from: bob})
         truffleAssert.eventEmitted(txResult, 'Burn', (ev) => {
           assert.equal(ev.user, bob, "error user");
           assert.equal(ev.qlcAddr, vaildNep5Addr, "error hash");
         ...
         await chai.assert.isRejected(this.instance.burn(invaildNep5Addr, burnAmount, {from: bob}), 'invalid qlc address');
    });
    ...
    it("circuitBraker", async () => {
      let qlcHash = "0x4c5aac8222797c7a85f7d015510ce8232f1229e0d3c487f60f74058486065e4e"
      ...
      await chai.assert.isRejected(this.instance.mint(mintAmount, qlcHash,  signHex), '');
      await this.instance.circuitBraker({from: tom})
      await chai.assert.isRejected(this.instance.mint(mintAmount, qlcHash,  signHex), 'invalid signature'); 
    })
